...
export const crearEstudiante = async (datos) => {
  ...
  const usuarioCheck = await pool.query("SELECT id_usuario FROM usuarios WHERE id_usuario = $1 AND estado = true", [
    id_usuario,
  ])
  ...
  const preexistente = await pool.query(
...
export const crearPreinscripcion = async (datos) => {
  return await crearEstudiante({ ...datos, estado: "Pendiente" })
}
